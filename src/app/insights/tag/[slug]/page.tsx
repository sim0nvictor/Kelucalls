...
import { Clock, Eye, ArrowLeft, BookOpen, Tag } from "lucide-react";
...
async function getArticlesByTag(
  ...
  const articles = (data ?? [])
    .flatMap((row) => {
      const articleValue = (row as { article?: unknown }).article;
      const article = Array.isArray(articleValue)
        ? (articleValue[0] as ArticleWithRelations | null)
        : (articleValue as ArticleWithRelations | null);

      return article && article.status === "published" && (!article.published_at || new Date(article.published_at) <= new Date())
        ? [article]
        : [];
    })
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!tag) {
    return { title: "Tag Not Found | Kelucalls Insights" };
  }

  return {
    title: `${tag.name} | Kelucalls Insights`,
    description: `Browse all articles tagged with ${tag.name} on Kelucalls Insights.`,
  };
}
...
export default async function TagPage({ params, searchParams }: PageProps) {
  ...
  const { articles, total } = await getArticlesByTag(tag.id, limit, offset);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <header className="mb-12">
        <div className="mb-4 inline-flex items-center justify-center rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-2">
          <Tag className="mr-2 size-5 text-cyan-400" />
          <span className="text-lg font-medium text-cyan-300">{tag.name}</span>
        </div>
      ...
      {articles.length > 0 ? (
        <>
          ...
          {totalPages > 1 && (
            <div className="mt-12 flex justify-center gap-2">
              {pageNum > 1 && (
                <Link
                  href={`/insights/tag/${slug}?page=${pageNum - 1}`}
        ...
        <div className="py-12 text-center">
          ...
          <p className="text-slate-400">No articles with this tag yet.</p>
        </div>
      )}
    </div>
  );
